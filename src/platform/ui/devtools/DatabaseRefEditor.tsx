...
import {
  ...
  type DatabaseRefColumn,
  type DatabaseRefDatabase,
  type DatabaseRefFile,
  type DatabaseRefTable,
...
function cloneFile(file: DatabaseRefFile): DatabaseRefFile {
  return JSON.parse(JSON.stringify(file)) as DatabaseRefFile;
}

function uniqueEn(prefix: string, existing: string[]): string {
...
export function DatabaseRefEditor({ data, onChange, onSave, saveStatus }: DatabaseRefEditorProps) {
  ...
  const handleAddDatabase = () => {
    const next = cloneFile(data.databases.length ? data : emptyDatabaseRefFile());
    const nameEn = uniqueEn(
      'new_database',
  ...
  const handleSave = () => {
    if (!window.confirm('Save changes to database_ref.json? This updates the shared schema file.')) return;
    void onSave();
  };
